...
export interface BrollPlanOutput {
  continuityToken: string;            // short shared anchor — e.g. "amber-hero-001"
...
export async function planBrollShots(env: Env, input: BrollPlanInput): Promise<BrollPlanOutput> {
  ...
  const planPrompt = [
    ...
    `      "motionHint": "<1 sentence describing the motion if we animate this shot to 5s — e.g. 'slow drone push-in toward the subject'>",`,
    `      "durationSeconds": 5`,
    `    }`,
    `  ]`,
    `}`,
