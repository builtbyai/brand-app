...
export interface Env {
  ...
  // Vectorize — separate indexes per concept (brand fingerprints, competitor positioning, lead trends, brand-OS sections).
  VEC_BRANDS?: VectorizeIndex;
  VEC_COMPETITORS?: VectorizeIndex;
  VEC_LEADS?: VectorizeIndex;
  VEC_BRAND_OS?: VectorizeIndex;

  // KV — lightweight side-store for provider capability cache, brief preset cache, image meta.
  CACHE?: KVNamespace;

  // AI Gateway slug — created in dashboard, e.g. "brand-os-ai".
  ...
  // Cookie scope — defaults to the host the worker is reached at; set in wrangler.toml
  // so brain.* and cmd.* share a session.
  SESSION_COOKIE_DOMAIN?: string;

  // Brand OS RAG — self-hosted Ollama via an authenticated bridge.
  OLLAMA_BRIDGE_SECRET?: string;       // auth bearer for the ollama-bridge-signaling worker
  OLLAMA_BRIDGE_URL?: string;      // override for the signaling endpoint
}
