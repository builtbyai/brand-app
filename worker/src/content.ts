...
export const articlesDb = {
  ...
  async upsert(env: Env, tenantId: string, input: Partial<ArticleRow> & { title: string; content: string; source_url?: string }): Promise<ArticleRow> {
    const now = nowIso();
    const url = input.source_url ?? `urn:brand-os:${tenantId}:${slugify(input.title)}:${crypto.randomUUID()}`;
  ...
  async ingestFromUrl(env: Env, tenantId: string, srcUrl: string, sourceId: string | null, opts?: { category?: string; badge?: string }): Promise<ArticleRow> {
    ...
    // Fetch raw HTML/text
    const res = await fetch(url, { headers: { "user-agent": "brand-os/1.0 (+example.com)" } });
...
export const battlecardsDb = {
  ...
  async generate(env: Env, tenantId: string, input: { competitorDomain: string; objection: string; category: string }): Promise<BattlecardRow> {
    const prompt = `You are a sales-enablement strategist for Northwind (a team-productivity software studio).
  ...
  for (const r of rs.results ?? []) {
    ...
    try {
      const res = await env.AI.run("openai/gpt-4o-mini" as any, {
        messages: [
          { role: "system", content: "Extract structured intel from a business/industry article. Return ONLY JSON: {\"sentiment\":\"positive|neutral|negative\",\"angle\":\"competitor|industry|customer-pain|regulatory|other\",\"topic\":\"<3-5 word tag>\",\"signal\":\"<one sentence on why this matters to Northwind>\"}." },
 ...
 * are clearly off-topic for Northwind (a team-productivity CRM). Runs in batches
 * to keep the worker under per-tick wallclock + AI Gateway quota.
 *
 * Heuristic before LLM: bail fast if the title/description is short and lacks
 * any productivity/sales/CRM keyword — that alone catches most spam.
 */
export async function runRelevanceScrub(
  ...
  const KW = /productiv|workflow|automat|team|project|task|pipeline|crm|sales|lead|outreach|onboard|report|dashboard|analytics|integration|collaborat|ops|operation|saas|kpi|retention|adoption|manager|process/i;
  ...
  for (const r of rows) {
    ...
    try {
      const res = await env.AI.run("openai/gpt-4o-mini" as any, {
        messages: [
          { role: "system", content: "Classify whether an article is relevant to a team-productivity CRM business (Northwind). Reply with ONE token: RELEVANT or OFFTOPIC. Productivity, project management, sales and CRM, team collaboration, workflow automation, SaaS operations, field-service workflows — all RELEVANT. Cooking, celebrity gossip, gaming, fashion — OFFTOPIC." },
          { role: "user", content: `Title: ${r.title}\nDescription: ${r.description ?? "(none)"}` },
        ],
        max_tokens: 4,
      } as any, { gateway: { id: env.AI_GATEWAY_SLUG || "default" } } as any) as any;
...
async function fetchSourceItems(s: ContentSourceRow): Promise<string[]> {
  if (s.kind === "rss" || s.kind === "sitemap") {
    const res = await fetch(s.url, { headers: { "user-agent": "brand-os-ingest/1.0" } });
  ...
  if (s.kind === "reddit") {
    ...
    const res = await fetch(url, { headers: { "user-agent": "Mozilla/5.0 (compatible; brand-os-bot/1.0; +https://app.example.com)" } });
    if (!res.ok) throw new Error(`fetch ${url} → ${res.status}`);
  ...
  if (s.kind === "competitor") {
    ...
    const res = await fetch(s.url, { headers: { "user-agent": "Mozilla/5.0 (compatible; brand-os-bot/1.0)" } });
    if (!res.ok) throw new Error(`fetch ${s.url} → ${res.status}`);
    const html = await res.text();
