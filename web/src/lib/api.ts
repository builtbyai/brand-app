...
// Optional cross-origin API base — set at build time via VITE_API_BASE.
// When unset, API calls hit the same origin (the worker route catches them).
// Used while app.example.com DNS is still propagating: build with
// VITE_API_BASE=https://brand-os-api.example.workers.dev so brand-os-app.pages.dev
// can talk to the worker cross-origin.
export const API_BASE: string = (import.meta as any).env?.VITE_API_BASE ?? "";

/** Build a URL for an /api/* path, picking up cross-origin VITE_API_BASE if set. */
...
export const api = {
  ...
  // ----- Brand OS -----
  listBrandSections: (brand?: string) =>
    jsonFetch<{ sections: Array<{
      id: string; parent_brand: string | null; ord: number; title: string;
      slug: string; rel_path: string; word_count: number; updated_at: string;
    }> }>(`/api/brand-os/sections${brand ? `?brand=${encodeURIComponent(brand)}` : ""}`),
  getBrandSection: (id: string) =>
    jsonFetch<{ section: {
      id: string; parent_brand: string | null; ord: number; title: string;
      slug: string; rel_path: string; body_md: string; body_hash: string;
      word_count: number; updated_at: string; ingested_at: string;
    } }>(`/api/brand-os/sections/${encodeURIComponent(id)}`),
  searchBrandOs: (query: string, brand?: string, topK = 5) =>
    jsonFetch<{ hits: Array<{
      sectionId: string; chunkIndex: number; parentBrand: string;
      excerpt: string; score: number;
    }> }>("/api/brand-os/search", {
      method: "POST",
      body: JSON.stringify({ query, brand, topK }),
    }),
  askBrandOs: (input: {
    question: string;
    ...
    jsonFetch<{
      answer: string;
      backendUsed: "gemini" | "ollama";
      modelUsed: string;
      latencyMs: number;
      citations: Array<{ sectionId: string; parentBrand: string; score: number; excerpt: string }>;
    }>("/api/brand-os/ask", {
      method: "POST",
      body: JSON.stringify(input),
    }),
  listBrandOsRuns: () =>
    jsonFetch<{ runs: Array<{
      id: number; started_at: string; finished_at: string | null;
      files_scanned: number | null; files_changed: number | null;
      chunks_upserted: number | null; status: string; error: string | null;
    }> }>("/api/brand-os/runs"),
  ...
  seedDefaultSources: () => jsonFetch<{ added: number; skipped: number; total: number }>("/api/sources/seed-defaults", { method: "POST" }),
  spend: () =>
    jsonFetch<{
      asOf: number;
      notice: string;
...
export function subscribeScheduleEvents(
  ...
  const es = new EventSource(`${API_BASE}/api/events/stream`, { withCredentials: true } as EventSourceInit);
