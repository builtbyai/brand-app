    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-xs text-nw-soft-white/60 mt-1">
          Direct to <a href="https://replicate.com" target="_blank" rel="noreferrer" className="text-nw-bronze underline">Replicate</a>.
          Outputs are mirrored into R2 + visible in Media Library and the Video Editor.
        </p>
      </div>

      {!hasToken && (
        <div className="nw-card p-4 border-yellow-700/40 bg-yellow-900/10 text-xs text-yellow-200">
          <strong>REPLICATE_API_TOKEN not set on the worker.</strong> Run
          <code className="ml-1 px-1.5 py-0.5 rounded bg-nw-warm-black/60 text-nw-bronze">wrangler secret put REPLICATE_API_TOKEN</code> from <code>worker/</code>.
        </div>
      )}

      <form onSubmit={submit} className="nw-glass-glow rounded-lg p-4 space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
          <label className="space-y-1 md:col-span-2">
            <div className="font-mono uppercase text-nw-soft-white/50">Model</div>
            <select
          ...
          {isCustom && (
            <>
              <label className="space-y-1">
                <div className="font-mono uppercase text-nw-soft-white/50">Owner</div>
                <input
              ...
              <label className="space-y-1">
                <div className="font-mono uppercase text-nw-soft-white/50">Model name</div>
                <input
        ...
        <div className="relative">
          <textarea
            ...
            className="w-full bg-nw-brown/40 border border-nw-bronze/20 rounded px-3 py-2 text-sm pr-16"
          />
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
          {aspectOptions.length > 0 && (
            <label className="space-y-1">
              <div className="font-mono uppercase text-nw-soft-white/50">Aspect</div>
          ...
          {durationOptions.length > 0 && (
            <label className="space-y-1">
              <div className="font-mono uppercase text-nw-soft-white/50">Duration</div>
          ...
          <label className="space-y-1">
            <div className="font-mono uppercase text-nw-soft-white/50">Seed</div>
            <input value={seed} onChange={(e) => setSeed(e.target.value)} placeholder="random"
                   className="w-full bg-nw-brown/40 border border-nw-bronze/20 rounded px-2 py-1.5" />
          </label>
          <label className="space-y-1">
            <div className="font-mono uppercase text-nw-soft-white/50">Negative</div>
            <input value={negativePrompt} onChange={(e) => setNegativePrompt(e.target.value)} placeholder="blurry, lowres"
                   className="w-full bg-nw-brown/40 border border-nw-bronze/20 rounded px-2 py-1.5" />
          </label>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-2 border-t border-nw-bronze/15">
          <FrameSlot
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <label className="flex items-center gap-1.5 text-[11px] text-nw-soft-white/70 cursor-pointer">
          ...
          <button
            ...
            className="flex items-center gap-1 text-[11px] text-nw-bronze hover:underline"
          >
            <Layers className="w-3 h-3" /> {batchOpen ? "hide batch" : "batch mode"}
          </button>
        </div>
        {polling && (
          <p className="text-[11px] text-nw-soft-white/50">
            Long-running prediction. The result will land in Media Library when ready — feel free to navigate away.
          </p>
        )}
        {batchOpen && (
          <div className="pt-3 border-t border-nw-bronze/15 space-y-2">
            <div className="font-mono uppercase text-[10px] text-nw-soft-white/50">Batch prompts — one per line, all run with current settings</div>
            <textarea rows={5} value={batchPrompts}
              ...
              className="w-full bg-nw-brown/40 border border-nw-bronze/20 rounded px-3 py-2 text-sm font-mono" />
            <button type="button" onClick={submitBatch} disabled={busy || !batchPrompts.trim()}
              className="flex items-center gap-2 bg-nw-bronze/15 hover:bg-nw-bronze/25 border border-nw-bronze/40 rounded px-3 py-1.5 text-nw-bronze text-xs disabled:opacity-50">
              <Sparkles className="w-3 h-3" /> Queue batch ({batchPrompts.split("\n").filter((s) => s.trim()).length})
            </button>
            {batchId && (
              <div className="text-[11px] text-nw-soft-white/60">
                Batch <span className="font-mono text-nw-bronze">{batchId}</span> · {jobsCtx.byBatch(batchId).filter((j) => j.status === "succeeded").length}/{jobsCtx.byBatch(batchId).length} done
              </div>
            )}
          </div>
        )}
      ...
      {results.length > 0 && (
        <div>
          <div className="text-xs font-mono uppercase text-nw-bronze mb-2">Recent generations</div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {results.map((r) => (
              <div key={r.predictionId} className="nw-glass rounded-lg overflow-hidden">
                {r.mediaType === "video" ? (
                  <video src={r.publicUrl} controls className="w-full bg-nw-warm-black" />
                ) : (
                  <img src={r.publicUrl} alt="" className="w-full bg-nw-warm-black" />
                )}
                <div className="p-3 text-[11px] space-y-1">
                  <div className="font-mono text-nw-bronze">{r.model}</div>
                  <div className="text-nw-soft-white/70 line-clamp-2" title={r.promptUsed}>
                    {r.promptUsed}
                  </div>
                  <div className="flex items-center justify-between pt-1">
                    <span className="text-nw-soft-white/40">{r.predictionId.slice(0, 12)}</span>
                    <a href={r.publicUrl} target="_blank" rel="noreferrer"
                       className="flex items-center gap-1 text-nw-bronze hover:underline">
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <div className="font-mono uppercase text-[10px] text-nw-soft-white/50">
          {label} {required && <span className="text-yellow-300">*</span>}
        </div>
        <div className="flex items-center gap-3 text-[11px]">
          <button type="button" onClick={onPick} className="text-nw-bronze hover:underline flex items-center gap-1">
      ...
      {url ? (
        <div className="relative w-full aspect-video rounded overflow-hidden border border-nw-bronze/20 bg-nw-warm-black">
          <img src={url} alt="" className="w-full h-full object-cover" />
        </div>
      ) : (
        <div className="w-full aspect-video rounded border border-dashed border-nw-bronze/20 bg-nw-warm-black/40 flex items-center justify-center text-[10px] text-nw-soft-white/40">
          empty
        </div>
      )}
    </div>
