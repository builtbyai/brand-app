...
export default function SendToScheduler({ open, onClose, mediaUrl, initialCopy = "" }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-nw-warm-black/80 backdrop-blur-sm flex items-center justify-center p-4" onClick={onClose}>
      <form
        onSubmit={submit}
        onClick={(e) => e.stopPropagation()}
        className="bg-nw-coffee border border-nw-bronze/30 rounded-lg w-full max-w-2xl flex flex-col"
      >
        <div className="flex items-center justify-between p-4 border-b border-nw-bronze/15">
          <div>
          ...
          <button type="button" onClick={onClose} className="text-nw-soft-white/40 hover:text-nw-soft-white"><X className="w-4 h-4" /></button>
        </div>

        <div className="p-4 grid md:grid-cols-[1fr_2fr] gap-4">
          <div className="aspect-square rounded overflow-hidden border border-nw-bronze/20">
            <img src={mediaUrl} alt="" className="w-full h-full object-cover" />
          </div>
          <div className="space-y-3 text-xs">
            <div className="grid grid-cols-2 gap-2">
              <select value={channelId} onChange={(e) => setChannelId(e.target.value)} className="bg-nw-brown/40 border border-nw-bronze/20 rounded px-2 py-1.5">
              ...
              <input type="datetime-local" value={when} onChange={(e) => setWhen(e.target.value)} className="bg-nw-brown/40 border border-nw-bronze/20 rounded px-2 py-1.5" />
            </div>
            <textarea rows={6} value={copy} onChange={(e) => setCopy(e.target.value)} placeholder="Post copy" className="w-full bg-nw-brown/40 border border-nw-bronze/20 rounded px-2 py-1.5" />
            ...
            {done ? (
              ...
              <button type="submit" disabled={busy || !channelId || !copy.trim()} className="bg-nw-bronze text-nw-warm-black font-semibold px-4 py-2 rounded disabled:opacity-50 flex items-center gap-2">
