...
export function Skeleton({ className = "", width, height }: SkeletonProps) {
  ...
  return <div className={`nw-skeleton ${className}`} style={style} />;
}
  ...
  return (
    <div className={`grid gap-3 grid-cols-2 sm:grid-cols-3 lg:grid-cols-${columns}`}>
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="nw-card overflow-hidden">
  ...
  return (
    <div className="space-y-2">
      {Array.from({ length: rows }).map((_, i) => (
        <div key={i} className="nw-card p-3 flex items-center gap-3">
