...
export default function CommandPalette({ open, onClose, onNavigate }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-nw-bg/70 backdrop-blur-sm flex items-start justify-center p-4 pt-[12vh]" onClick={onClose}>
      <div
        className="nw-glass-glow w-full max-w-xl overflow-hidden nw-fade-in"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center px-3 border-b border-nw-border">
          <Search className="w-4 h-4 text-nw-text-subtle" />
          <input
            ref={inputRef}
            value={q}
            ...
            className="flex-1 bg-transparent px-3 py-3.5 text-sm focus:outline-none placeholder:text-nw-text-subtle"
          />
          <kbd className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-nw-surface-2 border border-nw-border text-nw-text-muted">ESC</kbd>
        </div>
        <div className="max-h-[60vh] overflow-y-auto p-1.5">
          {filteredNav.length > 0 && (
            <>
              <div className="text-[10px] font-mono uppercase text-nw-text-subtle px-3 py-1.5 tracking-widest">Jump to</div>
              {filteredNav.map((n, i) => {
                const idx = i;
                const Icon = n.icon;
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-3 py-2 rounded-md text-left text-sm ${
                      activeIdx === idx ? "bg-nw-surface-2 text-nw-text" : "text-nw-text-muted hover:bg-nw-surface-1"
                    }`}
                  >
                    <Icon className={`w-4 h-4 ${activeIdx === idx ? "text-nw-bronze" : ""}`} />
                    <span className="flex-1">{n.label}</span>
                    <span className="text-[11px] text-nw-text-subtle hidden sm:inline">{n.hint}</span>
                    <ArrowRight className="w-3 h-3 text-nw-text-subtle" />
                  </button>
                );
              })}
            </>
          ...
          {dyn.length > 0 && (
            <>
              <div className="text-[10px] font-mono uppercase text-nw-text-subtle px-3 py-1.5 tracking-widest mt-2">Results</div>
              {dyn.map((d, j) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {d.subtitle && <div className="text-[11px] text-nw-text-subtle truncate">{d.subtitle}</div>}
                    </div>
                  </button>
                );
              })}
          ...
          {flat.length === 0 && (
            <div className="text-xs text-nw-text-subtle py-12 text-center">No matches</div>
          )}
        </div>
      </div>
    </div>
