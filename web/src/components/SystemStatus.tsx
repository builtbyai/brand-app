...
const CHECK_LABEL: Record<string, { label: string; fix?: { href?: string; cmd?: string } }> = {
  ...
  ai_gateway:           { label: "AI Gateway",       fix: { href: "https://dash.cloudflare.com/0000000000000000000000000000cf00/ai/ai-gateway" } },
...
export default function SystemStatus({ compact = false }: { compact?: boolean }) {
  ...
  if (compact) {
    if (loading) return <span className="text-[10px] font-mono text-nw-soft-white/40 flex items-center gap-1"><Loader2 className="w-3 h-3 animate-spin" /> checking…</span>;
    return (
  ...
  return (
    <div className="nw-glass rounded-lg p-4 space-y-3">
      <div className="flex items-center justify-between">
        ...
        <button onClick={load} className="text-nw-soft-white/50 hover:text-nw-soft-white">
          <RefreshCw className={`w-3.5 h-3.5 ${loading ? "animate-spin" : ""}`} />
        </button>
      </div>

      {loading && !data ? (
        <div className="flex items-center gap-2 text-xs text-nw-soft-white/50"><Loader2 className="w-3 h-3 animate-spin" /> loading…</div>
      ) : (
              ...
              <div key={id} className="flex items-start gap-2">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className={c.ok ? "text-nw-soft-white/90" : "text-nw-soft-white"}>{meta.label}</span>
                    {meta.fix?.href && !c.ok && (
                      <a href={meta.fix.href} target="_blank" rel="noreferrer" className="text-nw-bronze hover:underline text-[10px] flex items-center gap-0.5">
                        fix <ExternalLink className="w-2.5 h-2.5" />
                      </a>
                    )}
                  </div>
                  {!c.ok && (c.detail || meta.fix?.cmd) && (
                    <div className="text-[10px] text-nw-soft-white/50 mt-0.5 font-mono break-all">
