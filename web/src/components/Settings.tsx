  ...
  return (
    <div className="space-y-6">
      ...
      <div className="nw-glass rounded-lg p-4 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1 text-xs bg-nw-brown/40 border border-nw-bronze/20 px-3 py-1.5 rounded hover:bg-nw-brown/60 disabled:opacity-50"
            >
              {busy ? <Loader2 className="w-3 h-3 animate-spin" /> : <RefreshCw className="w-3 h-3" />} Run ingest now
            </button>
            <button
              onClick={() => setAdding((v) => !v)}
              className="flex items-center gap-1 text-xs bg-nw-bronze text-nw-warm-black font-semibold px-3 py-1.5 rounded"
            >
        ...
        {adding && (
          <div className="nw-glass-glow rounded p-3 space-y-2">
            <div className="grid md:grid-cols-2 gap-2">
              <input
                value={form.label} onChange={(e) => setForm({ ...form, label: e.target.value })}
                placeholder="Label, e.g. Northwind Blog"
                className="bg-nw-brown/40 border border-nw-bronze/20 rounded px-2 py-1.5 text-xs"
              />
              <select
            ...
            <input
              ...
              placeholder="https://… (RSS) or https://reddit.com/r/productivity/.json"
            ...
            <div className="flex gap-2">
              <button
                ...
                className="bg-nw-bronze text-nw-warm-black text-xs font-semibold px-4 py-1.5 rounded disabled:opacity-50"
              >Add</button>
              <button onClick={() => setAdding(false)} className="text-xs text-nw-soft-white/50 hover:text-nw-soft-white px-2">Cancel</button>
            </div>
          </div>
        )}
        ...
        {loading ? (
          ...
          <div className="text-xs text-nw-soft-white/40 py-4 text-center">No sources configured. Add one above.</div>
        ) : (
            ...
            {sources.map((s) => (
              <div key={s.id} className="flex items-start gap-3 py-2 text-xs">
                <button
                  ...
                  className={`mt-0.5 ${s.is_active ? "text-green-400" : "text-nw-soft-white/30"}`}
                >
                  <Power className="w-4 h-4" />
                </button>
                <div className="flex-1 min-w-0">
                  <div className="flex items-baseline gap-2">
                    <span className="font-mono uppercase text-[10px] text-nw-bronze">{s.kind}</span>
                    <span className="font-semibold">{s.label}</span>
                    <span className="text-[10px] text-nw-soft-white/40">· {s.category}</span>
                    {s.fail_count > 0 && (
                  ...
                  <div className="flex items-center gap-1 text-[10px] text-nw-soft-white/40 mt-0.5">
                    <a href={s.url} target="_blank" rel="noreferrer" className="text-nw-bronze hover:underline truncate flex items-center gap-1 max-w-md">
                      {s.url} <ExternalLink className="w-2.5 h-2.5 shrink-0" />
                    </a>
                  </div>
                  {s.last_run_at && (
                    <div className="text-[10px] text-nw-soft-white/40 mt-0.5">
                      last run: {new Date(s.last_run_at).toLocaleString()}
                      {s.last_error && <span className="text-red-400 ml-2">· {s.last_error.slice(0, 80)}</span>}
                    </div>
                  )}
                </div>
                <button onClick={() => remove(s.id)} className="text-nw-soft-white/40 hover:text-red-400">
