    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-xs text-nw-soft-white/60 mt-1">
          Chainable text → image → video pipeline. Use <code className="text-nw-bronze">{"{{prev_text}}"}</code>, <code className="text-nw-bronze">{"{{prev_image}}"}</code>, <code className="text-nw-bronze">{"{{prev_video}}"}</code> to splice upstream outputs into the next node's prompt. Video nodes automatically use the most recent upstream image as the start frame.
        </p>
      </div>

      <div className="nw-glass-glow rounded-lg p-4 flex items-center gap-2 flex-wrap">
        ...
        <button onClick={() => addNode("text")} className="flex items-center gap-1 text-xs text-nw-bronze hover:underline"><Plus className="w-3 h-3" /> Text</button>
        <button onClick={() => addNode("image")} className="flex items-center gap-1 text-xs text-nw-bronze hover:underline"><Plus className="w-3 h-3" /> Image</button>
        <button onClick={() => addNode("video")} className="flex items-center gap-1 text-xs text-nw-bronze hover:underline"><Plus className="w-3 h-3" /> Video</button>
        ...
        {nodes.map((n, i) => {
          ...
          return (
            <React.Fragment key={n.id}>
              ...
              />
              {i < nodes.length - 1 && (
                <div className="flex justify-center text-nw-bronze/50">
...
function NodeCard({
  ...
  const statusColor =
    ...
    status === "running" ? "text-nw-bronze" :
    status === "failed" ? "text-red-300" : "text-nw-soft-white/40";

  return (
    <div className="nw-glass rounded-lg overflow-hidden">
      <div className="flex items-center gap-2 p-2.5 border-b border-nw-bronze/10">
        <Icon className="w-4 h-4 text-nw-bronze" />
        <input
          value={node.label} onChange={(e) => onChange({ label: e.target.value })}
          className="flex-1 bg-transparent text-sm font-display font-bold text-nw-bronze focus:outline-none"
        />
        ...
        <button onClick={() => onMove(-1)} disabled={isFirst}
                className="text-nw-soft-white/50 hover:text-nw-bronze disabled:opacity-30">
      ...
      <div className="p-3 space-y-2 text-xs">
        <div className="relative">
          <textarea
            ...
            className="w-full bg-nw-brown/40 border border-nw-bronze/20 rounded px-2 py-1.5 font-mono pr-16"
          />
        ...
        {node.kind === "image" && (
          <div className="grid grid-cols-2 gap-2">
            <label className="space-y-1">
              <div className="font-mono uppercase text-[10px] text-nw-soft-white/50">Quality</div>
            ...
            <label className="space-y-1">
              <div className="font-mono uppercase text-[10px] text-nw-soft-white/50">Size</div>
        ...
        {node.kind === "video" && (
          <div className="grid grid-cols-3 gap-2">
            <label className="space-y-1">
              <div className="font-mono uppercase text-[10px] text-nw-soft-white/50">Model</div>
            ...
            <label className="space-y-1">
              <div className="font-mono uppercase text-[10px] text-nw-soft-white/50">Aspect</div>
            ...
            <label className="space-y-1">
              <div className="font-mono uppercase text-[10px] text-nw-soft-white/50">Seconds</div>
        ...
        {result && (
          <div className="pt-2 border-t border-nw-bronze/15">
            {result.status === "running" && (
              <div className="flex items-center gap-2 text-nw-bronze">
                <Loader2 className="w-3 h-3 animate-spin" /> running…
              </div>
            )}
            {result.status === "done" && result.text && (
              <div className="text-nw-soft-white/80 whitespace-pre-wrap">{result.text}</div>
            )}
            {result.status === "done" && result.publicUrl && result.kind === "image" && (
              <img src={result.publicUrl} alt="" className="max-w-full max-h-48 rounded border border-nw-bronze/20" />
            )}
            {result.status === "done" && result.publicUrl && result.kind === "video" && (
              <video src={result.publicUrl} controls className="max-w-full max-h-64 rounded border border-nw-bronze/20" />
            )}
