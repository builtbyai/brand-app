...
export default function GlobalChat({ activeTab }: { activeTab: string }) {
  ...
  const send = async (e?: React.FormEvent) => {
    ...
    const sys: Msg = {
      role: "system",
      content:
        `You are an assistant inside the Northwind Brand OS studio. ` +
    ...
    <>
      ...
      <button
        ...
        className={`fixed bottom-5 right-5 z-40 w-12 h-12 rounded-full shadow-lg flex items-center justify-center transition-all ${
          open
            ? "bg-nw-coffee border border-nw-bronze/40 text-nw-soft-white"
            : "bg-nw-bronze text-nw-warm-black hover:scale-105"
        }`}
      ...
      {/* Slide-over panel */}
      <div
        className={`fixed top-0 right-0 bottom-0 z-30 w-full sm:w-[420px] bg-nw-coffee border-l border-nw-bronze/20 shadow-2xl transition-transform duration-200 flex flex-col ${
          open ? "translate-x-0" : "translate-x-full"
        }`}
      >
        <div className="flex items-center justify-between p-3 border-b border-nw-bronze/15">
          <div>
            <div className="font-display font-bold text-sm flex items-center gap-2">
              <MessageSquare className="w-4 h-4" /> Chat
            </div>
            <div className="text-[10px] font-mono text-nw-soft-white/40">
          ...
          <div className="flex items-center gap-2">
            <select
              ...
              className="bg-nw-brown/40 border border-nw-bronze/20 rounded px-2 py-1 text-[10px]"
            >
              {MODELS.map((m) => <option key={m.id} value={m.id}>{m.label}</option>)}
            </select>
            {messages.length > 0 && (
              <button onClick={() => { setMessages([]); setErr(null); }} className="text-nw-soft-white/40 hover:text-nw-soft-white" title="reset">
                <RotateCcw className="w-3.5 h-3.5" />
              </button>
            )}
            <button onClick={() => setOpen(false)} className="text-nw-soft-white/40 hover:text-nw-soft-white">
              <X className="w-4 h-4" />
            </button>
          </div>
        </div>
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto p-3 space-y-2 text-sm">
          {messages.length === 0 && (
            <div className="text-center text-nw-soft-white/40 text-xs pt-8 space-y-2">
              <div className="text-nw-bronze font-mono">Try:</div>
            ...
            <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
              <div
                className={`max-w-[88%] rounded-lg px-3 py-2 whitespace-pre-wrap text-[13px] ${
                  m.role === "user"
                    ? "bg-nw-bronze text-nw-warm-black font-medium"
                    : "bg-nw-brown/60 border border-nw-bronze/15"
                }`}
              >
                {m.content}
              </div>
          ...
          {busy && (
            <div className="flex items-center gap-2 text-xs text-nw-soft-white/50">
        ...
        <form onSubmit={send} className="flex gap-2 p-3 border-t border-nw-bronze/15">
          <input
            value={input} onChange={(e) => setInput(e.target.value)}
            placeholder="Ask anything…"
            disabled={busy}
            className="flex-1 bg-nw-brown/40 border border-nw-bronze/20 rounded px-3 py-2 text-sm"
          />
          <button
            type="submit" disabled={!input.trim() || busy}
            className="bg-nw-bronze text-nw-warm-black font-semibold text-xs px-3 rounded disabled:opacity-50"
          >
