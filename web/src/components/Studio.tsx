...
export default function Studio() {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="nw-glass rounded-lg px-3 py-2 flex items-center gap-2 overflow-x-auto">
        <Wand2 className="w-4 h-4 text-nw-bronze flex-shrink-0" />
        <span className="text-[11px] font-mono uppercase text-nw-soft-white/60">Studio</span>
        <span className="text-nw-soft-white/40">/</span>
        {(["Build", "Refine", "Ship"] as const).map((g) => (
          <React.Fragment key={g}>
            <span className={`text-[10px] font-mono uppercase ${active.group === g ? "text-nw-bronze" : "text-nw-soft-white/40"}`}>{g}</span>
            <span className="text-nw-soft-white/30">·</span>
          </React.Fragment>
        ))}
        <span className="text-sm font-display font-bold text-nw-bronze">{active.label}</span>
        <span className="text-[11px] text-nw-soft-white/50 hidden md:inline">— {active.hint}</span>
        <button
          onClick={() => setInspectorOpen((v) => !v)}
          className="ml-auto text-[11px] text-nw-soft-white/60 hover:text-nw-bronze flex items-center gap-1"
        >
      ...
      <div className={`grid gap-3 ${inspectorOpen ? "grid-cols-1 lg:grid-cols-[56px_1fr_280px]" : "grid-cols-1 lg:grid-cols-[56px_1fr]"}`}>
        {/* LEFT RAIL */}
        <nav className="nw-glass rounded-lg p-1 flex lg:flex-col gap-1 overflow-x-auto">
          {VIEWS.map((v) => {
            const Icon = v.Icon;
            ...
            return (
              <button
                ...
                className={`flex-shrink-0 flex items-center justify-center w-12 h-12 rounded ${isActive ? "bg-nw-bronze text-nw-warm-black" : "text-nw-soft-white/60 hover:bg-nw-bronze/10 hover:text-nw-bronze"}`}
              >
        ...
        {/* RIGHT INSPECTOR */}
        {inspectorOpen && (
          <aside className="hidden lg:block nw-glass rounded-lg p-3 text-xs space-y-3 self-start sticky top-20">
            <div>
              <div className="font-mono uppercase text-[10px] text-nw-soft-white/50 mb-1">Tips for this workbench</div>
              <div className="text-nw-soft-white/80 leading-relaxed">{INSPECTOR_TIPS[view]}</div>
            </div>
            <div className="border-t border-nw-bronze/15 pt-3">
              <div className="font-mono uppercase text-[10px] text-nw-soft-white/50 mb-1">Jump to</div>
              <div className="grid grid-cols-2 gap-1">
                {(["Build", "Refine", "Ship"] as const).map((g) => (
                  <div key={g} className="col-span-2">
                    <div className="text-[10px] font-mono uppercase text-nw-bronze/70 mt-2">{g}</div>
                    <div className="grid grid-cols-1 gap-0.5">
                      {VIEWS.filter((v) => v.group === g).map((v) => (
                        <button key={v.id} onClick={() => setView(v.id)}
                          className={`text-left text-[11px] px-2 py-1 rounded ${v.id === view ? "bg-nw-bronze/20 text-nw-bronze" : "text-nw-soft-white/70 hover:bg-nw-bronze/10"}`}>
                          {v.label}
                        </button>
                      ))}
                    </div>
