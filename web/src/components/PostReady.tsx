...
//
// The grid mirrors the example.com/mocks/#grid Instagram Profile Grid:
//   - 3-col tight grid, 3px gaps, black behind
//   - tiles are square aspect, drag to reorder
//   - active drop target outlined in #DEB568 (matches our nw-bronze)
//
// What's different:
...
const NORTHWIND_CAPTION_SYSTEM = `You write Northwind branded social captions (LinkedIn / Facebook / Instagram). This is a high-impact, premium team-productivity voice. Follow the Northwind Caption Framework EXACTLY — same emoji, same line breaks. End with the literal token "3d" on its own line as the last line.
...
- status updates buried in chat threads
- handoffs that drop between sales, ops, and delivery
- slow reporting and manual copy-paste
- low tool adoption across the team
- unclear ownership and next steps
- duplicate data entry and stale records
- onboarding new teammates taking too long

FRAMEWORK (8 sections, in this order):

1. Symbolic opener — ⚜️ [HEADLINE 4-6 WORDS ALL CAPS ENDS WITH PERIOD.]  — premium, sharp, memorable
...
3. Product positioning — one sentence starting with Northwind. Define who it's for and what category it belongs to. Mention the unified workspace, tasks, or reporting.

4. Workflow explanation — one sentence framing Northwind as the OS of the whole workflow: "From [first thing] to [final thing], your team can [outcome]…"

5. Feature bullet stack — exactly 5 🟡 bullets, 5-9 words each, mapped to the chosen pain angle. Group around real workflow steps (intake / tasks / context / reports / status / approvals / handoff).

6. Closing slogan — ✦ [3 short imperative clauses, parallel verbs, the brand promise]. Default: "✦ Plan smart. Track clean. Ship fast." but you can vary if the pain angle warrants.

7. Brand signature — exactly: 🦅 Northwind | The Workspace Built to Win.

8. Hashtag cluster — single line with 9-12 hashtags. Always include #Northwind #TeamCRM #TeamSoftware. Add pain-relevant tags (e.g. #Productivity #Operations #Workflow #FieldSales #ProjectManagement #OpsTech).

End with "3d" on a line by itself.

Hard rules:
- No quotes, no markdown fences, no preamble.
- Do not change the structural emoji (⚜️ 🟡 ✦ 🦅).
- Mobile-friendly line spacing — blank lines between sections 1/2, 2/3, 4/5, 5/6, 7/8, 8/3d.
- Output ONLY the caption.`;

const NORTHWIND_FALLBACK_TEMPLATE = `⚜️ FROM INTAKE TO DELIVERY.

Plan faster.
Track cleaner.
Ship sooner.

Northwind gives teams one unified workspace built for real projects, real handoffs, and real reporting.
From the first task to the final sign-off, your team can track every step of the work in one clean workflow.

🟡 Real-time status across the whole team
🟡 Tasks, context, and files in one record
🟡 Automated reports built in seconds
🟡 Clear ownership and next steps
🟡 Approvals and handoffs in one place

✦ Plan smart. Track clean. Ship fast.
🦅 Northwind | The Workspace Built to Win.

#Northwind #TeamCRM #TeamSoftware #Productivity #Operations #Workflow #ProjectManagement #FieldSales #OpsTech
3d`;
...
export default function PostReady() {
  ...
  // Prefer northwindapp (exact match) → any northwind* → first channel.
  const preferredChannelId = (): string => {
    const exact = channels.find((c) => /^northwindapp$/i.test(c.display_name.trim()));
    if (exact) return exact.id;
    const looser = channels.find((c) =>
      /northwind/i.test(c.display_name) || /northwind/i.test(c.platform)
    );
  ...
  const promote = async (m: MediaItem) => {
    ...
    try {
      const res = await api.chat({
        messages: [
          { role: "system", content: NORTHWIND_CAPTION_SYSTEM },
          { role: "user", content: `Source: ${m.source}. Mime: ${m.mime}.\nDraft a caption for this generation following the Northwind template EXACTLY.` },
        ],
        max_tokens: 800,
      });
      setSelected((cur) => cur ? { ...cur, caption: res.content, status: "draft" } : cur);
    } catch (e: any) {
      setSelected((cur) => cur ? { ...cur, caption: NORTHWIND_FALLBACK_TEMPLATE, status: "draft" } : cur);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="text-nw-soft-white/50 text-xs flex items-center gap-2">
          <Loader2 className="w-3 h-3 animate-spin" /> loading media…
        </div>
      ) : ordered.length === 0 ? (
        <div className="nw-card p-6 text-center text-xs text-nw-soft-white/50">
      ...
      {selected && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="nw-glass-glow rounded-lg w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col">
            <div className="flex items-center gap-2 px-4 py-3 border-b border-nw-bronze/20">
              <Send className="w-4 h-4 text-nw-bronze" />
              <div className="text-sm font-display font-bold text-nw-bronze">Schedule post</div>
              <button onClick={() => setSelected(null)} className="ml-auto text-nw-soft-white/60 hover:text-nw-bronze">
                <X className="w-4 h-4" />
              </button>
            </div>
              ...
              <div className="space-y-3 text-xs">
                <label className="space-y-1 block">
                  <div className="flex items-center gap-2">
                    <div className="font-mono uppercase text-[10px] text-nw-soft-white/50">Caption</div>
                  ...
                  <textarea rows={14} value={selected.caption}
                    ...
                    className="w-full bg-nw-brown/40 border border-nw-bronze/20 rounded px-2 py-1.5 font-mono text-[11px] leading-relaxed" />
                  {selected.status === "drafting_caption" && (
                    <div className="flex items-center gap-1 text-nw-bronze text-[10px]">
                      <Loader2 className="w-3 h-3 animate-spin" /> auto-drafting via LLM
                    </div>
                  )}
                </label>
                <label className="space-y-1 block">
                  <div className="font-mono uppercase text-[10px] text-nw-soft-white/50">Channel</div>
                ...
                <label className="space-y-1 block">
                  <div className="font-mono uppercase text-[10px] text-nw-soft-white/50">Schedule for</div>
                ...
                <div className="flex items-center gap-2 pt-2 border-t border-nw-bronze/15">
                  ...
                  <button onClick={() => setSelected(null)}
                    className="text-[11px] text-nw-soft-white/60 hover:text-nw-bronze">close</button>
                </div>
              </div>
            </div>
          </div>
