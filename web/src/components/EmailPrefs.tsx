...
export default function EmailPrefs() {
  ...
  if (loading) return <div className="text-xs text-nw-soft-white/60 flex items-center gap-2"><Loader2 className="w-3 h-3 animate-spin" /> loading prefs…</div>;
  ...
  return (
    <div className="nw-glass rounded-lg p-4 space-y-4">
      ...
      <label className="block text-xs">
        <div className="font-mono uppercase text-nw-soft-white/50 mb-1">Notification email</div>
        <input
      ...
      <div className="space-y-2 text-xs">
        ...
        <label className="flex items-center gap-2">
          ...
          <span className="text-nw-bronze">$</span>
          <input
            ...
            placeholder="0 = off"
            className="w-20 bg-nw-brown/40 border border-nw-bronze/20 rounded px-2 py-1"
          />
        </label>
      </div>

      <div className="flex items-center gap-2">
        <button onClick={save} disabled={saving} className="bg-nw-bronze text-nw-warm-black text-xs font-semibold px-3 py-1.5 rounded disabled:opacity-50 flex items-center gap-1">
          {saving ? <Loader2 className="w-3 h-3 animate-spin" /> : <Save className="w-3 h-3" />}
          Save
        </button>
        <button onClick={test} disabled={!prefs.notify_email.trim()} className="bg-nw-brown/40 border border-nw-bronze/20 text-xs px-3 py-1.5 rounded disabled:opacity-50 flex items-center gap-1 hover:bg-nw-brown/60">
        ...
        {testStatus && <span className="text-xs text-nw-soft-white/70">{testStatus}</span>}
      </div>
    </div>
  );
}
