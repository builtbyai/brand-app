  ...
  return (
    <div className="fixed bottom-3 left-3 z-30">
      <div className="nw-glass rounded-lg border border-nw-bronze/30 shadow-lg overflow-hidden text-xs">
        <div className="flex items-center gap-2 px-3 py-1.5 border-b border-nw-bronze/15">
          <Layers className="w-3.5 h-3.5 text-nw-bronze" />
          <span className="font-display font-bold text-nw-bronze">Assets</span>
          <span className="text-nw-soft-white/40 font-mono ml-auto">{items.length}</span>
          <button onClick={() => setOpen((v) => !v)} className="text-nw-soft-white/60 hover:text-nw-bronze">
        ...
        {open && (
          <div className="p-2 flex gap-1.5 overflow-x-auto max-w-[680px]">
            {sorted.length === 0 ? (
              <div className="text-nw-soft-white/40 text-[10px] px-2 py-3">(no media yet)</div>
...
function AssetTile({ m, pinned, onTogglePin }: { m: MediaItem; pinned: boolean; onTogglePin: () => void }) {
  return (
    <div
      ...
      className="relative w-16 h-16 flex-shrink-0 rounded overflow-hidden border border-nw-bronze/20 hover:border-nw-bronze cursor-grab active:cursor-grabbing"
      title={m.source}
    >
      ...
      <button onClick={(e) => { e.stopPropagation(); onTogglePin(); }}
        ...
        className={`absolute top-0.5 right-0.5 w-4 h-4 rounded-full flex items-center justify-center z-10
          ${pinned ? "bg-nw-bronze text-nw-warm-black" : "bg-black/50 text-nw-bronze/80 hover:bg-nw-bronze/30"}`}>
