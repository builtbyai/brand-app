...
export default function Connections() {
  ...
  const connect = async (platform: string) => {
    ...
    try {
      ...
      // Pop a small window so the user stays on Brand OS mentally.
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={refresh}
          className="flex items-center gap-2 text-xs bg-nw-brown/40 hover:bg-nw-brown/60 border border-nw-bronze/20 px-3 py-1.5 rounded"
        >
      ...
      {loading ? (
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {PLATFORMS.map((p) => {
            ...
            return (
              <div key={p.id} className="nw-glass rounded-lg p-4">
                ...
                {linked.length === 0 ? (
                  <p className="text-xs text-nw-soft-white/50 mb-3">No accounts linked.</p>
                ) : (
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-1.5 bg-nw-bronze text-nw-warm-black text-xs font-semibold py-1.5 rounded disabled:opacity-50"
                >
                  {connecting === p.id ? (
