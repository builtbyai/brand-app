...
export default function WorkflowRunner() {
  const [brief, setBrief] = useState(() => {
    try { const seeded = sessionStorage.getItem("brand-os:prefilled-brief"); if (seeded) { sessionStorage.removeItem("brand-os:prefilled-brief"); return seeded; } } catch {}
      ...
      <section className="nw-card-raised p-4 space-y-3">
        ...
        <div className="flex flex-wrap items-center gap-3 text-xs">
          ...
          <button
            ...
            className="nw-btn-primary px-4 py-2 rounded-lg text-xs flex items-center gap-2 ml-auto disabled:opacity-50"
          >
          ...
          {workflowId && (
            <a
              ...
              className="text-nw-text-subtle font-mono"
              title={workflowId}
            >
          ...
          {totalSpent > 0 && (
            <span className="font-mono text-nw-bronze text-[11px]">
              spend ${totalSpent.toFixed(3)}
            </span>
          )}
        </div>
        {error && <div className="bg-nw-danger/10 border border-nw-danger/40 rounded p-2 text-xs text-nw-danger">{error}</div>}
      </section>

      {/* Live node graph */}
      <section className="nw-card-raised p-4 overflow-x-auto">
        <div className="text-[10px] font-mono uppercase tracking-widest text-nw-text-subtle mb-3">Pipeline execution</div>
        ...
        {/* Legend */}
        <div className="flex items-center gap-3 mt-3 text-[10px] font-mono text-nw-text-subtle">
          <span className="inline-flex items-center gap-1"><span className="nw-dot nw-dot-info" /> running</span>
          <span className="inline-flex items-center gap-1"><span className="nw-dot nw-dot-success" /> completed</span>
          <span className="inline-flex items-center gap-1"><span className="nw-dot nw-dot-warning" /> review</span>
          <span className="inline-flex items-center gap-1"><span className="nw-dot nw-dot-danger" /> failed</span>
        </div>
      </section>
      ...
      {assets.length > 0 && (
        <section>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
            {assets.map((a) => (
              <a key={a.id} href={a.uri} target="_blank" rel="noreferrer" className="nw-card overflow-hidden block hover:border-nw-border-strong transition-colors group">
                <div className="aspect-square bg-nw-surface-2 relative">
                  ...
                  {a.reviewScore != null && (
                    <div className={`absolute top-1 right-1 px-1.5 py-0.5 rounded text-[9px] font-mono font-bold ${
                      a.reviewScore >= 0.8 ? "bg-nw-success/90 text-nw-bg" :
                      a.reviewScore >= 0.6 ? "bg-nw-warning/90 text-nw-bg" :
                                              "bg-nw-danger/90 text-nw-bg"
                    }`}>
                      {Math.round(a.reviewScore * 100)}
                    </div>
                  )}
                </div>
                <div className="px-2 py-1.5 text-[10px] font-mono text-nw-text-subtle flex items-center justify-between">
...
function NodeCard({ node, rt }: { node: NodeDef; rt: NodeRuntime | undefined }) {
  ...
  const stateStyles: Record<string, { ring: string; dot: string; text: string }> = {
    idle:               { ring: "border-nw-border",        dot: "bg-nw-text-subtle",  text: "idle" },
    queued:             { ring: "border-nw-border",        dot: "bg-nw-text-subtle/60", text: "queued" },
    running:            { ring: "border-nw-info/60",       dot: "bg-nw-info animate-pulse", text: "running…" },
    completed:          { ring: "border-nw-success/40",    dot: "bg-nw-success",      text: "completed" },
    review_required:    { ring: "border-nw-warning/50",    dot: "bg-nw-warning",      text: "review" },
    failed_recoverable: { ring: "border-nw-warning/50",    dot: "bg-nw-warning",      text: "failed (recoverable)" },
    failed_terminal:    { ring: "border-nw-danger/50",     dot: "bg-nw-danger",       text: "failed" },
  };
  ...
  return (
    <div
      className={`nw-card p-2.5 border ${s.ring} transition-colors`}
      style={{ gridColumn: node.col + 1, gridRow: node.row + 1 }}
    >
      <div className="flex items-center gap-2">
        <Icon className={`w-3.5 h-3.5 shrink-0 ${state === "running" ? "text-nw-info" : state === "completed" ? "text-nw-success" : "text-nw-text-muted"}`} />
        <div className="flex-1 min-w-0">
          <div className="text-[11px] font-semibold truncate">{node.label}</div>
          <div className="flex items-center gap-1.5 mt-0.5">
            <span className={`inline-block w-1.5 h-1.5 rounded-full ${s.dot}`} />
            <span className="text-[10px] font-mono text-nw-text-subtle">{s.text}</span>
            {dur != null && state === "completed" && (
              <span className="text-[10px] font-mono text-nw-text-subtle ml-auto">{(dur / 1000).toFixed(1)}s</span>
            )}
...
function RunningTimer({ startedAt }: { startedAt: number }) {
  ...
  return <span className="text-[10px] font-mono text-nw-info ml-auto">{((now - startedAt) / 1000).toFixed(1)}s</span>;
}
