    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-xs text-nw-soft-white/60 mt-1">
          Live status of the multi-agent pipeline. Stubs live under <code className="text-nw-bronze">worker/src/nodes/</code>;
        ...
        <div className="flex gap-4 mt-3 text-[11px] font-mono">
          ...
          <span className="text-nw-soft-white/40">○ {design} design-only</span>
        </div>
      </div>

      {[1, 2, 3, 4].map((p) => (
        <div key={p}>
          <div className="text-xs font-mono uppercase text-nw-bronze mb-2">{PHASE_LABEL[p]}</div>
          <div className="space-y-1.5">
            {(byPhase.get(p) ?? []).map((n) => (
              <div key={n.id} className="nw-glass rounded p-3 flex items-start gap-3">
                <StatusIcon status={n.status} />
                <div className="flex-1 min-w-0">
                  <div className="flex items-baseline gap-2">
                    <span className="text-[10px] font-mono text-nw-soft-white/40">Node {n.id}</span>
                    <span className="text-sm font-medium">{n.name}</span>
                  </div>
                  {n.notes && <div className="text-[11px] text-nw-soft-white/60 mt-1">{n.notes}</div>}
                </div>
              </div>
            ))}
          </div>
...
function StatusIcon({ status }: { status: Status }) {
  ...
  return <AlertTriangle className="w-4 h-4 text-nw-soft-white/40 shrink-0 mt-0.5" />;
}
