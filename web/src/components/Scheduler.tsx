...
export default function Scheduler() {
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="flex items-center gap-2 text-nw-soft-white/60 text-sm">
        ...
        <div className="space-y-4">
          {byDay.map(([day, items]) => (
            <div key={day}>
              <div className="text-xs font-mono uppercase text-nw-bronze mb-2">{day}</div>
              <div className="space-y-2">
                {items.map((s) => {
                  const channel = channels.find((c) => c.id === s.channel_id);
                  return (
                    <div key={s.id} className="nw-glass rounded-lg p-3 flex items-center gap-3">
                      <StatusBadge status={s.status} />
                      <div className="flex-1 min-w-0">
                        <div className="text-xs font-mono uppercase text-nw-soft-white/70">
                          {channel ? `${channel.platform} · ${channel.display_name}` : s.channel_id}
                        </div>
                        <div className="text-[10px] text-nw-soft-white/50">
                      ...
                      {s.status !== "published" && s.status !== "cancelled" && (
                        <button
                          onClick={() => cancel(s.id)}
                          className="text-nw-soft-white/40 hover:text-red-400"
                          title="Cancel"
                        >
...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { Icon: any; color: string }> = {
    ...
    cancelled: { Icon: XCircle,      color: "text-nw-soft-white/30" },
  };
...
function Compose({ channels, onCreated }: { channels: Channel[]; onCreated: (s: Schedule) => void }) {
  ...
  return (
    <form onSubmit={submit} className="nw-glass-glow rounded-lg p-4 space-y-3">
      <div className="grid grid-cols-2 gap-3">
        <select
          value={channelId} onChange={(e) => setChannelId(e.target.value)}
          className="bg-nw-brown/40 border border-nw-bronze/20 rounded px-3 py-2 text-sm"
        >
      ...
      <button
        ...
        className="bg-nw-bronze text-nw-warm-black text-xs font-semibold px-4 py-2 rounded disabled:opacity-50"
      >
