...
export default function SceneComposer() {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="nw-glass-glow rounded-lg p-4 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 text-xs">
            <label className="space-y-1">
              <div className="font-mono uppercase text-nw-soft-white/50">Planner model</div>
            ...
            <label className="space-y-1">
              <div className="font-mono uppercase text-nw-soft-white/50">Scenes</div>
            ...
            <label className="space-y-1">
              <div className="font-mono uppercase text-nw-soft-white/50">Per-scene seconds</div>
          ...
          <label className="space-y-1 text-xs block">
            <div className="font-mono uppercase text-nw-soft-white/50">Style hints (optional)</div>
      ...
      {composition && (
        <>
          <div className="nw-glass rounded-lg p-4 space-y-2">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-sm font-display font-bold text-nw-bronze">{composition.title}</div>
                <div className="text-[11px] text-nw-soft-white/60 mt-0.5">{composition.styleSummary}</div>
              </div>
              <div className="flex items-center gap-2">
                <button onClick={buildAll} disabled={building}
                  className="flex items-center gap-1.5 bg-nw-bronze text-nw-warm-black font-semibold text-xs px-3 py-1.5 rounded disabled:opacity-50">
                  <Play className="w-3.5 h-3.5" /> {building ? "Building…" : "Build all scenes"}
                </button>
                <button onClick={reset}
                  className="text-[11px] text-nw-soft-white/60 hover:text-nw-bronze">new brief</button>
              </div>
            </div>
...
function SceneCard({
  ...
  return (
    <div className="nw-glass rounded-lg overflow-hidden">
      <div className="flex items-center gap-3 p-3 border-b border-nw-bronze/10">
        <span className="text-[10px] font-mono text-nw-soft-white/40">#{scene.idx + 1}</span>
        <input
        ...
        />
        <span className={`text-[10px] font-mono ${scene.status === "completed" ? "text-emerald-400" : scene.status === "failed" ? "text-red-300" : isWorking ? "text-nw-bronze" : "text-nw-soft-white/50"}`}>
          {statusLabel[scene.status] ?? scene.status}
        </span>
        <button onClick={onBuild} disabled={isWorking || scene.status === "completed"}
          className="flex items-center gap-1 text-[11px] bg-nw-bronze/15 hover:bg-nw-bronze/25 border border-nw-bronze/40 rounded px-2 py-1 text-nw-bronze disabled:opacity-50">
          {isWorking ? <Loader2 className="w-3 h-3 animate-spin" /> : <Play className="w-3 h-3" />}
          build
        </button>
        <button onClick={() => setOpen((v) => !v)} className="text-nw-soft-white/50 hover:text-nw-bronze">
...
function Field({ label, icon, children, right }: { label: string; icon?: React.ReactNode; children: React.ReactNode; right?: React.ReactNode }) {
  return (
    <label className="space-y-1 block">
      <div className="flex items-center gap-1 font-mono uppercase text-[10px] text-nw-soft-white/50">
        {icon}
        <span>{label}</span>
