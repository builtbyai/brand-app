    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => setWeekOffset(weekOffset - 1)} className="text-xs px-2 py-1 rounded bg-nw-brown/40 border border-nw-bronze/20 hover:bg-nw-brown/60">‹</button>
          <span className="text-xs font-mono">
            {weekStartDate.toLocaleDateString()} → {addDays(weekEndDate, -1).toLocaleDateString()}
          </span>
          <button onClick={() => setWeekOffset(weekOffset + 1)} className="text-xs px-2 py-1 rounded bg-nw-brown/40 border border-nw-bronze/20 hover:bg-nw-brown/60">›</button>
          <button onClick={() => setWeekOffset(0)} className="text-[10px] px-2 py-1 rounded bg-nw-brown/40 border border-nw-bronze/20 hover:bg-nw-brown/60 font-mono uppercase">this week</button>
          <button onClick={load} disabled={loading} className="text-nw-soft-white/50 hover:text-nw-soft-white">
      ...
      <div className="grid grid-cols-1 md:grid-cols-7 gap-2">
        {DAYS.map((day, i) => {
          ...
          const items = byDay[i] ?? [];
          return (
            <div key={day} className="nw-glass rounded-lg p-2 min-h-[160px]">
              <div className="flex items-baseline justify-between mb-2 px-1">
                <div className="text-[10px] font-mono uppercase text-nw-bronze">{day}</div>
                <div className="text-[10px] text-nw-soft-white/40">{d.getDate()}</div>
              </div>
              <div className="space-y-1.5">
                {items.length === 0 && (
                  <div className="text-[10px] text-nw-soft-white/30 px-1 py-2">no posts</div>
                )}
                {items.map((s) => {
                  const ch = channels.find((c) => c.id === s.channel_id);
                  const t = new Date(s.scheduled_for * 1000);
                  return (
                    <div key={s.id} className="bg-nw-brown/30 border border-nw-bronze/15 rounded p-1.5 text-[11px] group">
                      <div className="flex items-center justify-between">
                        <StatusBadge status={s.status} />
                        <span className="font-mono text-nw-soft-white/50">
                          {t.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
                        </span>
                      </div>
                      <div className="text-nw-soft-white/80 truncate mt-1">
                        {ch ? `${ch.platform}` : s.channel_id.slice(0, 8)}
                      </div>
                      {s.status !== "published" && s.status !== "cancelled" && (
                        <button
                          onClick={() => cancel(s.id)}
                          className="opacity-0 group-hover:opacity-100 text-nw-soft-white/40 hover:text-red-400 mt-1"
                          title="Cancel"
                        >
      ...
      <div className="text-[11px] text-nw-soft-white/50">
...
function StatusBadge({ status }: { status: string }) {
  ...
  if (status === "cancelled") return <XCircle className="w-3 h-3 text-nw-soft-white/30" />;
