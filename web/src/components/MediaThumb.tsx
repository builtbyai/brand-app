...
export default function MediaThumb({
  ...
  return (
    ...
    >
      ...
      {isAud && (
        <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-nw-bronze/30 to-nw-warm-black">
          <Music2 className="w-1/3 h-1/3 text-nw-bronze" />
        </div>
      )}
      {!isImg && !isVid && !isAud && (
        <div className="w-full h-full flex items-center justify-center bg-nw-warm-black/60">
          <FileQuestion className="w-1/3 h-1/3 text-nw-soft-white/30" />
        </div>
      )}
      ...
      {isPreviewing && (
        <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-nw-bronze/80 animate-pulse pointer-events-none" />
      )}
    </div>
  );
}
