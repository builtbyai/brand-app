    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-xs text-nw-soft-white/60 mt-1">
          Direct to <code className="text-nw-bronze">openai/gpt-image-2</code> via Cloudflare Workers AI.
          Transparent backgrounds auto-route to <code className="text-nw-bronze">openai/gpt-image-1.5</code>.
      ...
      <form onSubmit={submit} className="nw-glass-glow rounded-lg p-4 space-y-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
          <label className="space-y-1">
            <div className="font-mono uppercase text-nw-soft-white/50">Quality</div>
          ...
          <label className="space-y-1">
            <div className="font-mono uppercase text-nw-soft-white/50">Size</div>
          ...
          <label className="space-y-1">
            <div className="font-mono uppercase text-nw-soft-white/50">Format</div>
          ...
          <label className="space-y-1">
            <div className="font-mono uppercase text-nw-soft-white/50">Background</div>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <div className="font-mono uppercase text-[11px] text-nw-soft-white/50">
              Reference Images <span className="text-nw-soft-white/30">({refs.length}/16, optional — enables edit mode)</span>
            </div>
            <div className="flex items-center gap-3 text-[11px]">
              ...
              <label className="cursor-pointer text-nw-bronze hover:underline flex items-center gap-1">
          ...
          {refs.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {refs.map((r, i) => (
                <div key={i} className="relative w-16 h-16 rounded overflow-hidden border border-nw-bronze/20">
                  <img src={r.dataUrl} alt={r.name} className="w-full h-full object-cover" />
                  <button
                    type="button"
                    onClick={() => setRefs(refs.filter((_, j) => j !== i))}
                    className="absolute top-0 right-0 bg-nw-warm-black/80 text-red-300 p-0.5"
                    title="remove"
                  >
                    <X className="w-3 h-3" />
                  </button>
          ...
          <div className="pt-3 border-t border-nw-bronze/15 space-y-2">
            ...
            {batchId && (
              <div className="text-[11px] text-nw-soft-white/60">
                Batch <span className="font-mono text-nw-bronze">{batchId}</span> · {jobsCtx.byBatch(batchId).filter((j) => j.status === "succeeded").length}/{jobsCtx.byBatch(batchId).length} jobs done
              </div>
            )}
          </div>
        )}
      ...
      {results.length > 0 && (
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {results.map((r) => (
              <div key={r.id} className="nw-glass rounded-lg overflow-hidden">
                <img src={r.publicUrl} alt="" className="w-full bg-nw-warm-black" loading="lazy" />
                <div className="p-3 text-[11px] space-y-1">
                  ...
                  <div className="flex items-center justify-between pt-1">
                    <span className="text-nw-soft-white/40">{(r.bytes / 1024).toFixed(0)} KB</span>
