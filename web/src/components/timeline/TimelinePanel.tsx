...
export default function TimelinePanel({
  ...
  return (
    <div className="nw-glass rounded-lg overflow-hidden">
      {/* Top bar */}
      <div className="flex items-center gap-2 px-2.5 py-1.5 border-b border-nw-bronze/15 text-xs">
        <span className="font-mono uppercase text-[10px] text-nw-soft-white/50">Timeline</span>
        <span className="text-nw-soft-white/40 font-mono ml-2">{fmt(currentTimeSec)} / {fmt(totalDurationSec)}</span>
        <div className="ml-auto flex items-center gap-2">
          <ZoomOut className="w-3 h-3 text-nw-soft-white/40" />
          <input type="range" min={10} max={600} step={5} value={pxPerSec}
                 onChange={(e) => setPxPerSec(Number(e.target.value))}
                 className="w-32 accent-nw-bronze" />
          <ZoomIn className="w-3 h-3 text-nw-soft-white/40" />
          <span className="font-mono text-[10px] text-nw-soft-white/50 w-12 text-right">{pxPerSec}px/s</span>
        </div>
      </div>

      <div ref={containerRef} className="relative overflow-x-auto overflow-y-hidden bg-nw-warm-black/40">
        <div className="relative" style={{ width: totalWidthPx + LANE_LABEL_WIDTH }}>
          {/* Ruler */}
          <div
            className="relative border-b border-nw-bronze/20 cursor-pointer select-none"
          ...
          {/* Lanes */}
          {tracks.map((t) => (
            <div key={t.id} className="relative border-b border-nw-bronze/10"
                 style={{ height: t.height ?? 48 }}>
              <div className="absolute left-0 top-0 bottom-0 flex items-center justify-center text-[10px] font-mono text-nw-bronze border-r border-nw-bronze/20 bg-nw-warm-black/60"
                ...
                {clips.filter((c) => c.trackId === t.id).map((c) => {
                  ...
                  return (
                    <div key={c.id}
                      ...
                      className={`rounded border ${selected ? "border-nw-bronze ring-1 ring-nw-bronze shadow" : "border-nw-bronze/40"} cursor-grab hover:brightness-110`}
                    >
                      <div className="px-1.5 py-0.5 text-[10px] font-mono text-nw-warm-black truncate flex items-center justify-between">
                      ...
                      <div onMouseDown={(e) => startClipDrag(e, c, "resize_start")}
                           className="absolute left-0 top-0 bottom-0 w-1.5 cursor-ew-resize bg-nw-warm-black/30" />
                      <div onMouseDown={(e) => startClipDrag(e, c, "resize_end")}
                           className="absolute right-0 top-0 bottom-0 w-1.5 cursor-ew-resize bg-nw-warm-black/30" />
                    </div>
                  );
                })}
              </div>
          ...
          >
            <div className="absolute -top-0.5 -left-1.5 w-3 h-3 rounded-full bg-nw-bronze" />
          </div>
        </div>
      </div>

      <div className="flex items-center gap-2 px-2.5 py-1.5 border-t border-nw-bronze/15 text-[10px] font-mono text-nw-soft-white/50">
...
function RulerTicks({ totalDurationSec, pxPerSec }: { totalDurationSec: number; pxPerSec: number }) {
  ...
  return (
    <>
      {ticks.map((t) => (
        <div key={t} className="absolute top-0 bottom-0 border-l border-nw-bronze/30" style={{ left: t * pxPerSec }}>
          <div className="absolute top-1 left-1 text-[9px] font-mono text-nw-soft-white/70 pointer-events-none">{fmt(t)}</div>
        </div>
      ))}
    </>
  );
