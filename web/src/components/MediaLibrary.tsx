    ...
    >
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-2 text-xs bg-nw-bronze text-nw-warm-black font-semibold px-3 py-1.5 rounded"
          >
      ...
      <div
        className={`border-2 border-dashed rounded-lg p-6 text-center text-xs transition-colors ${
          dragOver ? "border-nw-bronze bg-nw-bronze/10" : "border-nw-bronze/20 bg-nw-brown/20"
        }`}
        onClick={() => fileRef.current?.click()}
        style={{ cursor: "pointer" }}
      >
        <div className="text-nw-soft-white/70">
          <Upload className="w-5 h-5 mx-auto mb-2 text-nw-bronze" />
      ...
      {jobs.length > 0 && (
        <div className="nw-glass rounded-lg p-3 space-y-2">
          <div className="flex items-center justify-between text-xs">
            <span className="font-mono">
              {activeCount > 0 && <span className="text-nw-bronze">{activeCount} active · </span>}
              <span className="text-green-400">{doneCount} done</span>
              {failedCount > 0 && <span className="text-red-400"> · {failedCount} failed</span>}
            </span>
            <button onClick={clearFinished} className="text-nw-soft-white/40 hover:text-nw-soft-white">clear finished</button>
          </div>
          <div className="max-h-48 overflow-y-auto space-y-1">
            {jobs.map((j) => (
              <div key={j.id} className="flex items-center gap-2 text-xs">
                {j.status === "queued" && <Loader2 className="w-3 h-3 text-nw-soft-white/40" />}
                {j.status === "uploading" && <Loader2 className="w-3 h-3 animate-spin text-nw-bronze" />}
                ...
                <span className="text-nw-soft-white/40 font-mono text-[10px]">
      ...
      {loading ? (
        <div className="text-nw-soft-white/60 text-sm flex items-center gap-2">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
          {items.map((m) => {
            return (
              <div key={m.id} className="nw-glass rounded-lg overflow-hidden flex flex-col">
                ...
                <div className="p-2 text-[10px] font-mono">
                  <div className="truncate text-nw-soft-white/70" title={m.r2_key}>{m.source}</div>
                  <div className="text-nw-soft-white/40">
                    {m.bytes ? (m.bytes / 1024).toFixed(0) + " KB" : "—"}
                  </div>
                  <div className="flex justify-between mt-1">
                    <a href={m.public_url} target="_blank" rel="noreferrer" className="text-nw-bronze hover:underline flex items-center gap-1">
                      <ExternalLink className="w-3 h-3" /> open
                    </a>
                    <button onClick={() => remove(m.id)} className="text-nw-soft-white/40 hover:text-red-400">
