...
import BrandOsWorkspace from "./components/BrandOsWorkspace";
...
import {
  ...
  History, TrendingUp, Users, FileText, Mail, DollarSign, Database, Server, Film, Home,
} from "lucide-react";
...
export default function App() {
  ...
  const NAV: NavSection[] = [
    ...
    {
      id: "brand_os", label: "Brand OS", icon: BookOpen,
    },
    {
  ...
  // Deep-link: #brand-os/<sectionId> → switch to brand_os tab (BrandOsWorkspace reads hash too)
  useEffect(() => {
    const apply = () => {
      const h = window.location.hash || "";
      if (h.startsWith("#brand-os/")) setActiveTab("brand_os");
    };
    apply();
  ...
  const titleFor = (id: string): React.ReactNode => {
    const flat: Record<string, string> = {
      ...
      brand_os: "Brand OS Command Center",
      dashboard: "Northwind Systems · Command Center",
    ...
    return <h1 className="font-display font-bold text-sm text-nw-text">{flat[id] ?? id}</h1>;
  };
  ...
  return (
    <div className="min-h-screen flex">
      <Sidebar
        ...
        footer={
          <div className="space-y-1">
            <button onClick={() => setPaletteOpen(true)} className="nw-nav-item">
              <kbd className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-nw-surface-2 border border-nw-border text-nw-text-muted">⌘K</kbd>
      ...
      <div className="flex-1 flex flex-col min-w-0">
              ...
              >
                {activeTab === "content_hub" && (
                  <ContentHub
                    ...
                    onGenerateVisuals={(art) => {
                      ...
                      try { sessionStorage.setItem("brand-os:prefilled-brief", brief); } catch {}
                ...
                {activeTab === "workflow_studio" && (
                  <div className="space-y-4">
                    <div className="nw-card p-4 border-yellow-700/40 bg-yellow-900/10">
                      <div className="text-xs text-yellow-300/90">
                        <strong>Legacy canvas.</strong> Cosmetic — does not use the new audit-ledger pipeline.
                        Use <button onClick={() => setActiveTab("generations")} className="underline text-nw-bronze">Studio › Generate</button> for the real flow.
                      </div>
                    </div>
                    <WorkflowStudio />
                  </div>
                ...
                {activeTab === "brand_os" && <BrandOsWorkspace />}
                {activeTab === "dashboard" && <Dashboard onJump={(id) => setActiveTab(id as Tab)} />}
        ...
        <footer className="border-t border-nw-border bg-nw-surface-0/60 px-4 sm:px-6 lg:px-8 py-2 text-[10px] font-mono uppercase tracking-widest text-nw-text-subtle flex items-center justify-between">
          <span>© 2026 NORTHWIND SYSTEMS · BRAND OS COMMAND CENTER</span>
          <span className="hidden sm:flex items-center gap-3">
            <span className="inline-flex items-center gap-1.5"><span className="nw-dot nw-dot-info" /> EDGE · CLOUDFLARE</span>
