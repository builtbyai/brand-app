...
| `src/index.css` | Tailwind v4 `@theme` block with `nw-*` design tokens — **the rebrand surface**. |
...
Current Brand OS production maps to:
...
| Worker name | `brand-os-api` (Cloudflare Workers) |
| D1 database | `brand-os-prod` (id `00000000-0000-0000-0000-000000000000`) |
| R2 bucket | `brand-os-media` |
| KV namespace | `CACHE` (id `0000000000000000000000000000abcd`) |
| Queues | `brand-os-publish` + `brand-os-publish-dlq` |
| Durable Object | `ScheduleRoom` class, binding `SCHEDULE_ROOM` |
| Vectorize indexes | `brand-os-brands`, `brand-os-competitors`, `brand-os-leads` (768 dim, cosine) |
| Workers AI | binding `AI`, gateway slug `brand-os-ai` |
...
| Old | New (example) |
|---|---|
| `brand-os-api` | `acme-api` |
| `brand-os-prod` | `acme-prod` |
| `brand-os-media` | `acme-media` |
| `brand-os-publish` | `acme-publish` |
| `brand-os-publish-dlq` | `acme-publish-dlq` |
| `brand-os-brands` / `-competitors` / `-leads` | `acme-brands` / `-competitors` / `-leads` |
| `brand-os-ai` (AI gateway slug) | `acme-ai` |
| `app.example.com` | `app.acmeco.com` |
| `example.com` (zone_name) | `acmeco.com` |
| `social.example.com` | your Postiz host (or remove the two POSTIZ_* vars if not using Postiz) |
| `database_id = "fd02…"` | the new D1 id from step 3.2 |
| `kv_namespaces id = "99fa…"` | the new KV id from step 3.2 |
| `R2_ACCOUNT_ID = "ca20…"` | your CF account id |
...
- `web/src/index.css` — the `@theme` block defines every `nw-*` color, font, radius, and shadow used across the app. Swap the hex values and you've rebranded.
- `web/src/components/LoginGate.tsx` — hardcoded `NORTHWIND.` wordmark and `Intelligence Studio` tagline.
- `web/src/App.tsx` — the `NAV` array (~30 tabs) — keep or prune per the new company's scope.

`nw-` is a stylistic design-token prefix; the simpler path is to keep the prefix and just retint the colors. Renaming the prefix is a global find-replace across `web/src/` and `web/src/index.css`.
...
- `web/README` / component structure — frontend conventions (single-page tab shell, `api.ts` as the contract, Tailwind v4 tokens).
