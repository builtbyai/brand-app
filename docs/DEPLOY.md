# Brand OS — Deployment Runbook

End-to-end. Assumes you already have wrangler authed against your Northwind Tech Cloudflare account and the Postiz stack on a self-hosted host reachable at `social.example.com`.
...
ssh a self-hosted host "cd /path/to/postiz && grep image: docker-compose.yml"
...
Brand OS's worker assumes a small set of `/public-api/v1/...` routes. Confirm they exist on your pinned image:

```bash
# From a self-hosted host, with Postiz API key in $POSTIZ_KEY:
...
# D1
wrangler d1 create brand-os-prod
# → paste the resulting `database_id` into wrangler.toml

# R2
wrangler r2 bucket create brand-os-media
...
wrangler vectorize create brand-os-brands       --dimensions=768 --metric=cosine
wrangler vectorize create brand-os-competitors  --dimensions=768 --metric=cosine
wrangler vectorize create brand-os-leads        --dimensions=768 --metric=cosine

# AI Gateway — create in dashboard, NOT via wrangler. Then paste slug into
# wrangler.toml [vars] AI_GATEWAY_SLUG = "brand-os-ai"
...
#   1. Dashboard → AI → AI Gateway → brand-os-ai → "Providers" tab
...
# Queue
wrangler queues create brand-os-publish
wrangler queues create brand-os-publish-dlq

# Apply schema
wrangler d1 migrations apply brand-os-prod --remote
```

Get an **R2 S3 API token** from the Cloudflare dashboard → R2 → Manage R2 API Tokens → "Object Read & Write" scoped to `brand-os-media`. Note Access Key ID + Secret.

## 3. Cloudflare Access — lock the Postiz tunnel to Worker only

1. Zero Trust dashboard → Access → Tunnels — confirm a tunnel exists for a self-hosted host exposing Postiz.
...
3. Zero Trust → Access → Applications → Add Application → Self-hosted.
   ...
   - Create a Service Token → name it `brand-os-worker`. Save **Client ID** and **Client Secret** — these are the `CF_ACCESS_CLIENT_ID` / `CF_ACCESS_CLIENT_SECRET` secrets the Worker uses.
...
# Frontend (Cloudflare Pages, name = brand-os)
cd ../web
npm install
npm run build
wrangler pages deploy dist --project-name=brand-os --branch=main
```

Verify the deployed bundle hash against your local `dist/` build before switching traffic.
...
Verify:
```bash
# From a self-hosted host, after triggering a post:
...
wrangler d1 execute brand-os-prod --remote --file=infra/seed-admin.sql
# (after editing seed-admin.sql with the produced hash + salt)
```

You can also reuse an existing admin row from another database by re-inserting it here with the same `salt` and `password_hash`.
...
| Failed jobs | `wrangler d1 execute brand-os-prod --remote --command "SELECT * FROM job_log WHERE status='dead' ORDER BY created_at DESC LIMIT 20"` |
| Postiz upgrade | `ssh a self-hosted host 'cd postiz && docker compose pull && docker compose up -d'` — then re-run §1 audit |
...
- **Tunnel flap on a self-hosted host** — the cron in `worker/src/cron.ts` reconciles every minute as a fallback, so a 5-minute tunnel outage at most delays status by a minute, not loses it.
